 ...
 *
 * Format used on debug functions is the same as `printf()`. Check source code
 * for usage examples Debug calls will be enabled or disabled automatically
 * before compiling according defined `DEBUG_LEVEL`.
 *
...
#define INFO                                                                   \
  3 ///< @brief Debug level that will give error, warning and info messages
#define DBG                                                                    \
  4 ///< @brief Debug level that will give error, warning,info AND dbg messages
...
#define setTagDebugLevel(tagName, level)                                       \
  debugTagManager.setTagLevel(tagName, level)
#define setTagToDefaultDebugLevel(tagName)                                     \
  debugTagManager.setTagToDefaultLevel(tagName)
...
#define DEBUG_LINE_PREFIX(TAG)                                                 \
  DEBUG_ESP_PORT.printf(PSTR("[%6lu][H:%5lu][%s:%d] %s(): ["), millis(),       \
                        (unsigned long)ESP.getFreeHeap(),                      \
                        extractFileName(__FILE__), __LINE__, __FUNCTION__);    \
  DEBUG_ESP_PORT.print(TAG);                                                   \
  DEBUG_ESP_PORT.print("] ");

#if DEBUG_LEVEL >= VERBOSE
#define DEBUG_VERBOSE(TAG, text, ...)                                          \
  if (debugTagManager.getTagLevel(TAG) >= VERBOSE) {                           \
...
#define DEBUG_DBG(TAG, text, ...)                                              \
  if (debugTagManager.getTagLevel(TAG) >= DBG) {                               \
...
#define DEBUG_INFO(TAG, text, ...)                                             \
  if (debugTagManager.getTagLevel(TAG) >= INFO) {                              \
...
#define DEBUG_WARN(TAG, text, ...)                                             \
  if (debugTagManager.getTagLevel(TAG) >= WARN) {                              \
...
#define DEBUG_ERROR(TAG, text, ...)                                            \
  if (debugTagManager.getTagLevel(TAG) >= ERROR) {                             \
...
#define DEBUG_VERBOSE(TAG, format, ...)                                        \
  if (debugTagManager.getTagLevel(TAG) >= VERBOSE) {                           \
    ESP_LOGV(TAG, "[H:%6d][S:%d T:%s] " format, ESP.getFreeHeap(),             \
             uxTaskGetStackHighWaterMark(NULL), pcTaskGetName(NULL),           \
             ##__VA_ARGS__);                                                   \
  }
#define DEBUG_DBG(TAG, format, ...)                                            \
  if (debugTagManager.getTagLevel(TAG) >= DBG) {                               \
    ESP_LOGD(TAG, "[H:%6d][S:%d T:%s] " format, ESP.getFreeHeap(),             \
             uxTaskGetStackHighWaterMark(NULL), pcTaskGetName(NULL),           \
             ##__VA_ARGS__);                                                   \
  }
#define DEBUG_INFO(TAG, format, ...)                                           \
  if (debugTagManager.getTagLevel(TAG) >= INFO) {                              \
    ESP_LOGI(TAG, "[H:%6d][S:%d T:%s] " format, ESP.getFreeHeap(),             \
             uxTaskGetStackHighWaterMark(NULL), pcTaskGetName(NULL),           \
             ##__VA_ARGS__);                                                   \
  }
#define DEBUG_WARN(TAG, format, ...)                                           \
  if (debugTagManager.getTagLevel(TAG) >= WARN) {                              \
    ESP_LOGW(TAG, "[H:%6d][S:%d T:%s] " format, ESP.getFreeHeap(),             \
             uxTaskGetStackHighWaterMark(NULL), pcTaskGetName(NULL),           \
             ##__VA_ARGS__);                                                   \
  }
#define DEBUG_ERROR(TAG, format, ...)                                          \
  if (debugTagManager.getTagLevel(TAG) >= ERROR) {                             \
    ESP_LOGE(TAG, "[H:%6d][S:%d T:%s] " format, ESP.getFreeHeap(),             \
             uxTaskGetStackHighWaterMark(NULL), pcTaskGetName(NULL),           \
             ##__VA_ARGS__);                                                   \
  }
#else
#define DEBUG_VERBOSE(TAG, format, ...)                                        \
  if (debugTagManager.getTagLevel(TAG) >= VERBOSE) {                           \
    ESP_LOGV(TAG, "[H:%6d][T:%s] " format, ESP.getFreeHeap(),                  \
             pcTaskGetName(NULL), ##__VA_ARGS__);                              \
  }
#define DEBUG_DBG(TAG, format, ...)                                            \
  if (debugTagManager.getTagLevel(TAG) >= DBG) {                               \
    ESP_LOGD(TAG, "[H:%6d][T:%s] " format, ESP.getFreeHeap(),                  \
             pcTaskGetName(NULL), ##__VA_ARGS__);                              \
  }
#define DEBUG_INFO(TAG, format, ...)                                           \
  if (debugTagManager.getTagLevel(TAG) >= INFO) {                              \
    ESP_LOGI(TAG, "[H:%6d][T:%s] " format, ESP.getFreeHeap(),                  \
             pcTaskGetName(NULL), ##__VA_ARGS__);                              \
  }
#define DEBUG_WARN(TAG, format, ...)                                           \
  if (debugTagManager.getTagLevel(TAG) >= WARN) {                              \
    ESP_LOGW(TAG, "[H:%6d][T:%s] " format, ESP.getFreeHeap(),                  \
             pcTaskGetName(NULL), ##__VA_ARGS__);                              \
  }
#define DEBUG_ERROR(TAG, format, ...)                                          \
  if (debugTagManager.getTagLevel(TAG) >= ERROR) {                             \
    ESP_LOGE(TAG, "[H:%6d][T:%s] " format, ESP.getFreeHeap(),                  \
             pcTaskGetName(NULL), ##__VA_ARGS__);                              \
  }
...
#define LOG_ERROR_IF_ZERO(TAG, ERROR_CODE, format, ...)                        \
  if (debugTagManager.getTagLevel(TAG) >= ERROR) {                             \
    if (ERROR_CODE == 0) {                                                     \
      DEBUG_ERROR(TAG, "Code: 0. " format, ##__VA_ARGS__);                     \
    }                                                                          \
  }

#define LOG_IF_CODE(ERROR_LEVEL, TAG, ERROR_CODE, CODE, format, ...)           \
  if (ERROR_CODE == CODE) {                                                    \
    DEBUG_##ERROR_LEVEL(TAG, "Code: %d. " format, ERROR_CODE, ##__VA_ARGS__);  \
  }
